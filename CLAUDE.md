...
- **Privacy queue schema**: columns `messageId, guildId, channelId, userId, categories, warnedAt, dueAt, scoldMessageId`, created on demand. On schema change, bump the `queueTableName` version suffix (e.g. `v3` → `v4`) in the Central Config node of **all three** workflows — both moderation workflows write it, and the Mai Chat workflow reads it (`userId` lookup, any guild) so Mai turns aggressive toward a currently-flagged user, wherever they talk to her (DMs included).
