...
The bot is **Mai**, the server's cat-moderator. Mentioning her, replying to her messages, or sending her a direct message gets an in-character reply (OpenAI, cat persona, short per-channel memory). Direct messages skip moderation (a bot cannot delete a DM) and are only accepted from users who share an allowlisted server with her. Her conversation memory is kept a few hours to give her context, then deleted; it is encrypted at rest (AES-256-GCM). While a member has an un-enforced violation, Mai turns aggressive toward them wherever they talk to her. She also reacts to trigger words (🐟, 😺), uses the server's own emotes rather than inventing codes for them, and can welcome new members ([mai/README.md](mai/README.md)). A few times a day she gets the **zoomies** for four minutes, which shows up in her status, in how readily she reacts and in the tone of her replies but never in how she moderates (`ZOOMIES_CHANCE=0` switches it off), and once in a rare while she puts a paw on a message for no reason at all and does nothing else with it. Where the operator set a `GIPHY_API_KEY` and the server ran `/mod config set gifs:true`, she can also search for a GIF and hang it on her reply: the model writes a search term and never an address, and only a GIPHY media host is ever posted.

Everything she says (persona, prompts, scold lines, welcome messages, reaction triggers, both sets of presence statuses) lives in [mai/config/mai.yaml](mai/config/mai.yaml). Secrets, models and limits live in `.env`.
