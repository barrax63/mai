...
- Guild allowlist via `DISCORD_GUILD_IDS` (comma-separated IDs; empty = all guilds). Direct messages are never moderated — a bot cannot delete a DM, so there is nothing to enforce.
