...
- **Shadow mode** runs both layers and acts on neither: no warning reaction, no scold reply, no queue row, no strike, no timeout. Every verdict goes to the log channel as a *Nur beobachtet* entry carrying the categories, the **top score** and a working jump link, so a server picks its threshold by reading its own traffic instead of by deleting the wrong messages and apologising. It is not a pause: rows queued before it was switched on are still enforced (`/mod off` is the pause).
  ...
  **The week ends with a number.** Every message classified during the period is counted into a per-guild score histogram (twenty buckets, top score only, no id and no timestamp: the same aggregate-only rule `shadow_hits` follows), and when the window closes Mai reads the worst 1% off it, sets `threshold` to that value, and says so in the closing entry with the sample count behind it and an undo button on it. This is the number the observation period was always run to find out, and it used to be chosen from a documentation line amounting to "start around 0.2 and watch", which meant finding out you were wrong by deleting things people meant. Counted for *every* classified message rather than only the flagged ones, because a histogram of what already cleared the line cannot show that the line is too high, which is exactly the German case. She refuses to guess in four situations: fewer than 200 messages, a percentile below 0.15 (a polite server must not be taught that its mildest 1% is a violation), one above 0.6, and a server that has already set a threshold itself.
...
- **`/mai ask` screens the question** ([src/moderation/screen.js](src/moderation/screen.js)), before the completion runs, because the answer quotes it back into the channel, that command was otherwise a way to publish arbitrary text past moderation under Mai's name. The verdict is the **stricter** of the provider's own and the guild's `threshold`: unlike every other path, where a threshold above 0 replaces the provider's `flagged` entirely, here a server's line may only tighten the screen. A guild that raised its threshold must not end up loosening the one guard that fails closed, and a German server that lowered it to 0.2 was previously being screened against a line its own messages are never near.
...
- **Chat** ([src/gateway/events/mai-chat.js](src/gateway/events/mai-chat.js), [src/chat/reply.js](src/chat/reply.js)): mentioning the bot, replying to one of its messages, or sending it a direct message makes Mai answer in character (`OPENAI_CHAT_MODEL` via `POST /chat/completions`; a **reasoning** model needs `OPENAI_REASONING_EFFORT=none` or it refuses every request that carries `tools`, while a model that does not reason refuses the parameter itself, which is why it has no default). A typing indicator runs while the model call is in flight, and all pings inside the reply are suppressed via `allowedMentions` plus `@everyone`/`@here` neutralization.
  ...
  - **Tools** (`CHAT_TOOLS_ENABLED`, [src/chat/tools.js](src/chat/tools.js)): `get_my_violations`, `get_server_info`, `get_current_time`, `get_my_timeout_status`, `get_my_appeal_status`, `get_server_emotes`, and `get_server_rules` where `chat.rules` in the YAML is filled in (an empty list means the tool is not offered at all, because one that answers "no rules" sends the model straight back to inventing them). **Almost no tool takes arguments from the model**: who is asking and where comes from the interaction context, so a model cannot read another member's record by naming them. `search_gif` is the single exception and takes a search term, which names no target and is validated before it is used. They are also synchronous apart from that one, so the timeout tool reads the member cache and reports "unknown" on a miss rather than guessing. At most two tool rounds, then the last call goes out without tools so she has to answer in words.
    - **GIFs** (`GIPHY_API_KEY` plus `/mod config set gifs:true`, off by default, [src/chat/gif-search.js](src/chat/gif-search.js)): `search_gif` is how Mai sends one. There is deliberately no curated list: she searches GIPHY and posts the result as an **embed image** on her reply, because a URL in the message body is rendered as a link *and* unfurled underneath it, which put a raw address in the middle of her sentence. That needs **Embed Links** in the channel, checked at startup for a guild that has GIFs on.
      - **What constrains it**: the model emits a **query and never an address**, so a prompt-injected model can steer the search but cannot post a link it invented. The query is normalized (a real string, whitespace collapsed, at most 50 characters, no URL smuggled in) and then screened by the same fail-closed classifier that guards `/mai ask`, because a member can steer it and the result is published in Mai's name. Only a URL on a `giphy.com` media host is posted, so an unexpected answer cannot become an arbitrary link in a channel. And it fails to nothing: a timeout (4 s), a quota, a dead provider all mean no GIF and a reply that goes out anyway.
      - **Housekeeping**: results are cached per query for 15 minutes and rate-limited twice, per guild (10 per 10 minutes) and process-wide (60 per hour), because a GIPHY beta key allows 100 searches an hour for the whole bot and a per-guild limit alone would stop being one as soon as there are three servers. `GIF_SEARCH_RATING` in [src/config.js](src/config.js) is the provider-side content filter. Every embed carries a *Powered by GIPHY* footer, which their terms ask for. A marker like `[GIF]` that the model narrated is stripped from the text, a GIF she sent without saying anything is posted alone rather than under the fallback line, and her memory keeps `(schickt ein GIF)` instead of the link.
      - **Not Tenor**: Google stopped accepting new Tenor API clients in January 2026 and shut the API down entirely on 30 June 2026, so that provider is not an option regardless of what older integration guides say. Everything provider-specific sits in four places in `gif-search.js` (the endpoint, the query parameters, the host allowlist, the one result field that is read), because this is evidently a category of dependency that disappears.
    - **Emotes**: unicode emoji are persona (`chat.persona` in the YAML tells her to use them and which fit which mood); the server's own emotes are a lookup, because the id in `<:name:id>` is the part nobody can guess and Discord does not fail on a wrong one, it posts the code as literal text. `get_server_emotes` returns up to 40 of them (sorted by name, unavailable ones left out), and the finished reply is swept for codes that do not resolve where it is going (`pruneInventedEmotes` in [src/ai/chat.js](src/ai/chat.js): this guild's emotes inside a guild, every reachable emote in a DM, and anything unverifiable dropped rather than posted broken).
...
- **Petting** (`/mai pet`, [src/chat/petting.js](src/chat/petting.js)): two strokes get a purr, two more get a warning, the next one bites and leaves her sulking with that member for ten minutes, during which she answers neither their mentions (they get the sulking emoji instead, because plain silence reads as a broken bot) nor their `/mai ask`. Per member and always their own doing, in memory, forgotten after five quiet minutes, and it never reaches moderation: a sulking Mai deletes and counts strikes exactly as before. Free, too: no model call, no database write, no Discord round trip, so the command answers synchronously.
- **Hairball**: while a guild's classifier is failing (the same state that puts a *degraded* entry in its log channel), `chat.hairballDirective` is added to her prompt in that guild and she answers snuffly, grumpy and half a step behind. It is a **mood and not a status report**: the directive forbids her explaining it, naming technology, errors, outages or moderation, or saying that anything is broken at all, and a test asserts those words are still in the config. "I cannot moderate right now" in a public channel is an invitation, the guild's staff were already told properly, and the free heuristics (invites, links, mass mentions, floods) keep working through a classifier outage anyway, so an off day is not a green light. Per guild, so one sick server does not make her ill in another, and a DM is never under the weather.
...
| Command | Who |
|---------|-----|
| `/ping`, `/mai ask <frage>`, `/mai pet`, `/mai forget`, `/mai appeal` | everyone |
...
| `timeout-ladder` | `0,5,15,30,60` | Timeout minutes per strike; the last step repeats. Also accepts `gentle`, `normal` or `firm`, which are stored as their minutes |
...
- **New reaction trigger / new wording**: edit `config/mai.yaml`. No code change, no rebuild if the file is bind-mounted. No handler may hold a literal string Mai says. A trigger may leave `pattern` out, which matches every message; there can be only one of those and it has to be the last entry, because nothing below it could ever be reached.
